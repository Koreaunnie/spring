...
import com.example.spring240924.dto.c21.*;
...
public class Controller21 {
    ...
    // @ModelAttribute 를 붙인 메소드 아규먼트(객체)는
    // request parameter 가 객체의 property 에 binding 됨
    ...
    // 클래스명을 lowerCamelCase 로 바꾼 이름으로 결정됨
    @GetMapping("sub6")
    public void g6(@ModelAttribute("book") MyBean6 a, Model model) {
    ...
    @GetMapping("sub7")
    public void g7(MyBean6 a, Model model) {
        System.out.println("a = " + a);
        // jsp : ${book.title}
        // Object m = model.getAttribute("book");
        Object m = model.getAttribute("myBean6");
    ...
    // sub8.jsp : 전송 버튼 클릭 시 post 방식으로 sub9 경로로 요청
    // sub9.jsp : model 에 있는 attribute 의 각 property 를 출력
    @GetMapping("sub8")
    public void g8() {
    }
...
//        m.setScore(3.14);
    ...
    // 없으면 객체를 생성한 후 사용
    // 이때 사용하는 생성자는 유일한 생성자 이거나 기본 생성자
